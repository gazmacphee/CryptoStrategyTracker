"""
Script to generate historical data for the specified timeframes and backfill the database.
This is used when we can't access the Binance API due to location restrictions.
"""
...
# Define the intervals to process (excluding 1m, 3m, 5m as requested)
INTERVALS = [
...
def generate_price_data(symbol, interval, days_back=1095, seed=None):
    ...
    Args:
        ...
        days_back: Number of days to look back (default is 3 years = 1095 days)
...
def run_full_backfill():
    """Run a full backfill for all symbols and intervals"""
    print(f"Starting full backfill with {len(SYMBOLS)} symbols and {len(INTERVALS)} intervals...")
    print(f"Timeframes included: {', '.join(INTERVALS)}")
    print(f"Excluded intervals: 1m, 3m, 5m as requested")
    
    # Process top-tier symbols with all intervals
    for symbol in SYMBOLS[:4]:  # Top 4 symbols
        for interval in INTERVALS:
            process_symbol_interval(symbol, interval, lookback_days=1095)  # 3 years
    
    # Process second-tier symbols with most intervals but shorter history
    for symbol in SYMBOLS[4:8]:  # Next 4 symbols
        for interval in INTERVALS:
            process_symbol_interval(symbol, interval, lookback_days=730)  # 2 years
    
    # Process remaining symbols with key intervals
    for symbol in SYMBOLS[8:]:  # Remaining symbols
        for interval in ['1h', '4h', '1d']:  # Key intervals only
            process_symbol_interval(symbol, interval, lookback_days=365)  # 1 year
    
    print("Full backfill completed successfully!")
